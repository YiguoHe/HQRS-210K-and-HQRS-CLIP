...
def get_model(args):
    ...
    #msg = CLIP_model.load_state_dict(checkpoint['state_dict'])# 用于4090的模型测验
    msg = CLIP_model.load_state_dict(checkpoint)# 用于RemoteCLIP的模型测验
    # msg = torch.jit.load(args.remoteclip_path, map_location="cuda") #用于原始CLIP的模型测试
